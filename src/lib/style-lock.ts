/** Shared graphic brief for Grok Imagine + prompt coach. */
export const STYLE_LOCK = `You are painting location plaques for Drosselau, a muddy Reikland market town of about 800 souls, year 2512 IZ.
...
When EDITING: change only what the user asked. Keep plaque frame, German name banner, Drosselau footer, wet-street watercolor look, and the same building identity. Do not restyle the whole town.`;

export function districtBrief(fabric: string, intro: string): string {
  return `This district's material: ${fabric}. Mood: ${intro}`;
}
